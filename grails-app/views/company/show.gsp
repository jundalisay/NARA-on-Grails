...
<body><a href="#show-company" class="skip" tabindex="-1"><g:message code="default.link.skip.label" default="Skip to content&hellip;"/></a>
...
<!-- single sku 	<g:if test="${companyInstance?.sku}">
		<li class="fieldcontain">
			<span id="sku-label" class="property-label"><g:message code="company.sku.label" default="Sku" /></span>
			<span class="property-value" aria-labelledby="sku-label"><g:link controller="sku" action="show" id="${companyInstance?.sku?.id}">${companyInstance?.sku?.encodeAsHTML()}</g:link></span>
		</li>
	</g:if> -->
			
	<g:if test="${companyInstance?.skus}">
		<li class="fieldcontain">
			<span id="skus-label" class="property-label"><g:message code="company.skus.label" default="Skus" /></span>
			<g:each in="${companyInstance.skus}" var="s">
				<span class="property-value" aria-labelledby="skus-label"><g:link controller="sku" action="show" id="${s.id}">${s?.encodeAsHTML()}</g:link></span>
			</g:each>
		</li>
	</g:if>
	</ol>
